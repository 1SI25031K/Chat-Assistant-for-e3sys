...
|:--|:---|:---|:---|:---|
| **F-01** | **Listener** | **宮本航聖** | I/O (Input) | SlackからのWebhook受信、Payloadの正規化 |
| **F-02** | **Filter** | **蘇木** | Filter | ユーザー意図の判定 (Intent Classification) |
| **F-03** | **Persistence** | **宮本航聖** | DB | データの永続化、ステータス管理 |
| **F-04** | **Gen (Core)** | **宮本航聖** | Logic | Gemini API を使用したフィードバック生成 |
| **F-05** | **Archive** | **宮本航聖** | Archive | AWS DB への全ログ蓄積実装 |
| **F-06** | **Notify** | **宮本航聖** | I/O (Output) | Slackへのレスポンス送信 |
...
| カテゴリ | タスク内容 | 担当者 | 備考 |
| :--- | :--- | :--- | :--- |
| **Slack設定** | Event Subscriptionsの変更 (`message.channels`の追加) | **蘇木** | メンションなしでメッセージを拾うための権限変更 |
| **F-01 Listen** | `server.py` の修正。`app_mention` 以外のイベント受信対応 | **宮本航聖** | `event["type"] == "message"` の解析ロジック追加 |
| **F-02 Filter** | 質問キーワード（"?", "？", "ですか"）の検知ロジック実装 | **宮本航聖** | `any()` を用いたキーワードマッチングの実装 |
| **F-03〜F-05** | パイプライン統合とAWS DynamoDB接続の最終調整 | **宮本航聖** | 既存コードのクラウド環境への最適化 |
| **Infra** | `Dockerfile` の作成と AWS App Runner へのデプロイ | **宮本航聖** | ngrokを廃止し、永続的なURLを発行 |
| **Security** | Bot IDによる無限ループ除外ロジックの厳格化 | **宮本航聖** | 自分の発言に反応しないためのガードレール実装 |
| **Config** | `.env` 情報の AWS Secrets Manager への移行 | **宮本航聖** | セキュアな環境変数管理 |
...
| カテゴリ | タスク内容 | 担当者 | 備考 |
| :--- | :--- | :--- | :--- |
| **database.py** | 過去ログ取得機能の実装 | **宮本航聖** | DynamoDBから特定のチャンネルの最新10〜20件のメッセージを取得する get_recent_history 関数の追加。 |
| **main.py** | AIプロンプトへのコンテキスト注入 | **宮本航聖** | 取得した過去ログをプロンプトに結合し、「これまでの議論を踏まえた回答」を生成できるように generator.py への渡し方を修正。 |
| **server.py / notifier.py** | スレッド返信対応 | **宮本航聖** | 元発言に対して「スレッド」で返信するロジックへの変更。 |
| **filter.py** | 高度なフィルターロジック | **宮本航聖** | AIが反応すべきかどうかをより精密に判定するロジックの完成。 |
